...
```bash
# Go to: https://supabase.com (or Neon, or Railway Postgres)
# Sign up → Create project → Copy connection string
# Tables are created automatically on first cold start — no manual
# migration needed. sql/schema.sql and sql/supabase_migration.sql are
# reference copies of that same schema for anyone who prefers to
# provision by hand.
```
...
```bash
cp .env.example .env
# Fill in your credentials — see .env.example for the full list and
# what each variable is for (bot token, admin ID, DATABASE_URL,
# ENCRYPTION_KEY, PUBLIC_BASE_URL, Paystack keys, etc.)
```
...
DATABASE_URL="postgresql://test:test@localhost/testdb" pytest tests/
```
