...
async def log_ai_usage(user_id: int, usage_type: str = "messages", prompt_text: str = "") -> bool:
    """Log AI feature usage for rate limiting."""
    try:
        pool = await get_pool()
        async with pool.acquire() as conn:
            table = "ai_chat_usage" if usage_type == "messages" else "ai_image_usage"
            ...
            await conn.execute(
                f"INSERT INTO {table} (user_id, prompt, created_at) VALUES ($1, $2, NOW())",
    ...
    try:
        ...
        async with pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT prompt, created_at FROM ai_chat_usage "
...
async def ai_chat(user_id: int, message: str, is_anime_question: bool = False) -> Optional[str]:
    ...
    try:
        ...
        # Add last few prompts for context (simplified - just past user queries)
        for hist in reversed(history[-3:]):
            if hist.get('prompt'):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    if response_text:
                        # Log usage
                        await log_ai_usage(user_id, "messages", message)
