...
# Every outbound broadcast carries this promo footer, so the reverse image
# search feature gets surfaced to the whole user/group base regardless of
# what the broadcast is actually about. Two more rows are added dynamically
# per job in _broadcast_keyboard(): a fixed-price Premium Group paywall
# button (every broadcast) and an optional admin-assigned "Join Group/
# Channel" button (only if that specific job set one — see
# handlers/broadcast_handler.py's join-link step).
_IMGSEARCH_PROMO_TEXT = "\n\n🔍 Got a pic you want to trace? Just send me any photo — first search's on the house."
_IMGSEARCH_PROMO_BUTTON = InlineKeyboardButton("🔍 Try Reverse Image Search", callback_data="tools_imgsearch_info")


def _broadcast_keyboard(job: dict) -> InlineKeyboardMarkup:
    rows = [
        [_IMGSEARCH_PROMO_BUTTON],
...
async def _send_broadcast(bot: Bot, job: dict, chat_id: int):
    ...
    caption = f"{content}{_IMGSEARCH_PROMO_TEXT}" if content else _IMGSEARCH_PROMO_TEXT.strip()
