...
async def handle_pay_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payment_result and payment_result.get("status") == "success":
        ...
        context.user_data["image_search_payment_provider"] = provider
        context.user_data["image_search_payment_key"] = api_key
...
async def handle_verify_unlock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    provider = context.user_data.get("image_search_payment_provider", "paystack")
    api_key = context.user_data.get("image_search_payment_key")
    ...
    if result.get("status") == "success":
        ...
        context.user_data.pop("image_search_payment_provider", None)
        context.user_data.pop("image_search_payment_key", None)
        await _reveal_links(query, results)
    else:
