...
async def handle_go_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Initialize Paystack payment for BotStore premium (unlimited listings).
    ...
    if payment_result and payment_result.get("status") == "success":
        ...
        context.user_data["botstore_premium_payment_reference"] = payment_reference
...
After payment, click "Verify Payment" below to unlock unlimited listings.
"""
        await query.edit_message_text(
            payment_text,
...
async def verify_go_premium_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Verify BotStore premium payment and activate if successful"""
    query = update.callback_query
    user_id = update.effective_user.id

    payment_reference = context.user_data.get("botstore_premium_payment_reference")
    ...
    if payment_result.get("status") == "success":
        ...
        if success:
            await query.edit_message_text(
                f"{EMOJI_COLORS.get('success', '✅')} **Premium Activated!**\n\n"
                f"You can now submit unlimited listings. 🎉",
                reply_markup=keyboard_gen.main_menu()
            )
            context.user_data.pop("botstore_premium_payment_reference", None)
        else:
            await query.edit_message_text(
                f"{EMOJI_COLORS.get('error', '❌')} Payment verified but premium activation failed.\n"
