...
from config import ADMIN_ID, EMOJI_COLORS, BOT_TOKEN, PAYSTACK_SECRET_KEY, PAYSTACK_PUBLIC_KEY, DATABASE_URL, PUBLIC_BASE_URL, LOG_GROUP_ID
...
async def cmd_envcheck(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"PAYSTACK_PUBLIC_KEY: {mask(PAYSTACK_PUBLIC_KEY)}",
...
async def cmd_setpremium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    arrangements, or anyone the Paystack webhook couldn't reach.
...
async def cmd_confirmpay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: manually verify a Paystack reference and activate whatever
