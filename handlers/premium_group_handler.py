...
async def handle_premium_pay_init(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Already-paid bypass: this button is attached to every broadcast (see
    # api/cron_broadcast.py), so the same user will see it again and again —
    # in this group, in another group's broadcast, in an old message they
    # scroll back to, etc. has_paid() is intentionally global per user (there
    # is only one Premium Group for the whole bot — see module docstring),
    # so if they've already completed a "premium_group_join" payment through
    # ANY of those buttons, charging them again here would be a straight
    # duplicate charge. Check this BEFORE creating a new Paystack transaction.
    if await db.has_paid(user.id, "premium_group_join"):
        if PREMIUM_GROUP_INVITE_LINK:
            await safe_edit_message(query,
                f"{EMOJI_COLORS.get('success', '✅')} You've already paid for the Premium Group — no need to pay again.\n\n"
                f"Tap below to join:",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔗 Join Premium Group", url=PREMIUM_GROUP_INVITE_LINK)]
            ...
            await query.answer(
                "You've already paid — an admin will add you to the Premium Group shortly.",
                show_alert=True
            )
        return
    ...
    if payment_result and payment_result.get("status") == "success":
        ...
        await db.log_payment(
            ...
            payment_type="premium_group_join",
            chat_id=PREMIUM_GROUP_CHAT_ID or None,
        )
        context.user_data["premium_group_pay_ref"] = reference
        context.user_data["premium_group_pay_provider"] = provider
        context.user_data["premium_group_pay_key"] = api_key
...
async def handle_premium_pay_verify(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    provider = context.user_data.get("premium_group_pay_provider", "paystack")
    api_key = context.user_data.get("premium_group_pay_key")
    ...
    if result.get("status") == "success":
        ...
        context.user_data.pop("premium_group_pay_provider", None)
        context.user_data.pop("premium_group_pay_key", None)
