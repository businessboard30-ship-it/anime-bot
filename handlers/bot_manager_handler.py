...
async def show_bot_manager(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        await query.edit_message_text(text, reply_markup=_bots_keyboard(bots), parse_mode="Markdown")
    else:
...
async def show_bot_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not record:
        bots = await bot_manager.get_user_bots(user_id)
        await query.edit_message_text("⚠️ Bot not found.", reply_markup=_bots_keyboard(bots))
        return

    ok, info = await bot_manager.verify_bot_token(record["token"])
    if not ok:
        bots = await bot_manager.get_user_bots(user_id)
        await query.edit_message_text(f"⚠️ Couldn't reach this bot: {info}", reply_markup=_bots_keyboard(bots))
        return
...
async def confirm_remove_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bots = await bot_manager.get_user_bots(user_id)
    await query.edit_message_text("🗑️ Bot removed from BotManager.", reply_markup=_bots_keyboard(bots))
