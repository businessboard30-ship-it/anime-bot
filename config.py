...
# group). Required so handlers/moderation.py's handle_join_request can tell
# "this join request is for the paid group" apart from every other group that
# just has autodmjoin turned on — without this, join-request approval and
# payment verification are two disconnected systems and anyone with a
# request-to-join link gets auto-approved regardless of payment.
PREMIUM_GROUP_CHAT_ID = int(os.getenv("PREMIUM_GROUP_CHAT_ID", "0") or "0")
